...
# in-kernel unpack — no proposal round-trip, no merge kernel.  Replaced the
# segmented variant (fixed 2048-wide segment sorts + exhaustion merge):
# the sort's per-element cost grows only ~1.2x from 2048- to 8192-wide
# (probe), which undercuts the merge kernel + props traffic it deletes at
...
def _ascend_topk_pack_tail_dense_kernel(
    ...
    # the old scalar form was ~4.6x slower).  The kernel is BRANCHLESS on
...
def _ascend_topk_pack_tail_sub_kernel(
    ...
    # SUB-1 elements, scalar form (the proven pattern from the old tail
    # kernel).  Kept in its own kernel: sharing one with the tree-based
    # dense kernel breaks the tree lowering (measured).
    row_id = tl.program_id(0)
    if REPACK:
        skip = tl.load(totals_ptr + row_id * 8 + 3) != 3
    else:
...
def _ascend_topk_threshold_kernel(
    ...
    if row_len <= TOPK:
        ...
        if rem <= CAP:
            ...
            # Target 2x TOPK (capped at 3/4 CAP): with the sampler's spread
            # (sigma ~ 155-250 rows depending on SAMP) this keeps L' ~4 sigma
            # from both route-window edges; 1.5x TOPK sat only ~2.4 sigma
            # from the lower edge and fired fallbacks on ~1% of rows.
...
def _ascend_topk_compact_seg_kernel(
    ...
    # GM access anywhere (the old extract kernel's dominant cost), and no
    # ctz/vgather custom op anymore.
    pid = tl.program_id(0)
    row_id = pid // NSEG
    seg = pid % NSEG
    totals_ptr += row_id * 8
    ...
    # bases. (Folding the reduce into this kernel was measured a net
    # loss on this hardware: per-program rescans and same-address
    # atomics cost more than the 3us reduce kernel they replaced.)
    if NSEG == 1:
...
def _ascend_topk_sort_final_kernel(
    ...
    # (vbitsort + vmrgsort4 via the TLE custom ops) replaces the old
    # finalize's 32-pass bit descent + scalar scatter (~90ns/lane).
...
def _top_k_per_row_prefill(
    ...
    if not _TLE_IMPORT_OK:
        raise NotImplementedError(
            ...
            "(triton.experimental.tle); the pure-Triton fallback was removed"
        )
    if stride1 != 1:
